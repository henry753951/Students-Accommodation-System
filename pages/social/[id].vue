<template>
  <div class="flex flex-col h-screen">
    ...
    <header class="bg-white shadow-sm px-6 py-4 flex items-center justify-between">
      <h1 class="text-2xl font-bold">{{current_property_id.address }}</h1>
    </header>
    ...
    <div class="flex-1 grid grid-cols-12 gap-6 p-6">
      ...
      <div class="col-span-9 space-y-6">
        <div class="bg-white rounded-lg shadow-sm p-4">
          ...
          />
          <button @click="publishPost" class="absolute right-3 top-1/2 transform -translate-y-1/2">Post</button>           </div>
          ...
          <div class="space-y-4">
            ...
            <Avatar class="bg-slate-200 mr-3">        
              <AvatarImage
                :src="posts.value?.app_user && comment.app_user.avatar_url ? comment.app_user.avatar_url : 'null'"
                alt="@radix-vue"
              />
...
//貼文顯示資料
const { data: Datas, pending: isLoading, refresh: refreshPosts} = useAsyncData(async () => {
...
const newPost = ref('');
  ...
  const { data, error } = await supabase.from('posts').insert(
    {
      rental_property_id: current_property_id.value,
